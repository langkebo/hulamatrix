...
export declare class MemoryCryptoStore implements CryptoStore {
    ...
    getCrossSigningKeys(_txn: unknown, func: (keys: Record<string, CrossSigningKeyInfo> | null) => void): void;
    getSecretStorePrivateKey<K extends keyof SecretStorePrivateKeys>(_txn: unknown, func: (key: SecretStorePrivateKeys[K] | null) => void, type: K): void;
    storeSecretStorePrivateKey<K extends keyof SecretStorePrivateKeys>(_txn: unknown, type: K, key: SecretStorePrivateKeys[K]): void;
    countEndToEndSessions(_txn: unknown, func: (count: number) => void): void;
    getEndToEndSession(deviceKey: string, sessionId: string, _txn: unknown, func: (session: ISessionInfo) => void): void;
    getEndToEndSessions(deviceKey: string, _txn: unknown, func: (sessions: {
        [sessionId: string]: ISessionInfo;
    }) => void): void;
    storeEndToEndSession(deviceKey: string, sessionId: string, sessionInfo: ISessionInfo, _txn: unknown): void;
    /**
    ...
    getEndToEndInboundGroupSession(senderCurve25519Key: string, sessionId: string, _txn: unknown, func: (groupSession: InboundGroupSessionData | null, groupSessionWithheld: IWithheld | null) => void): void;
    storeEndToEndInboundGroupSession(senderCurve25519Key: string, sessionId: string, sessionData: InboundGroupSessionData, _txn: unknown): void;
    /**
    ...
    doTxn<T>(_mode: Mode, _stores: Iterable<string>, func: (txn?: unknown) => T): Promise<T>;
}
