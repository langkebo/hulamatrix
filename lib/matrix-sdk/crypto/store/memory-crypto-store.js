...
export class MemoryCryptoStore {
  ...
  getAccount(_txn, func) {
    func(this.account);
  }
  storeAccount(_txn, accountPickle) {
    this.account = accountPickle;
  }
  getCrossSigningKeys(_txn, func) {
    func(this.crossSigningKeys);
  }
  getSecretStorePrivateKey(_txn, func, type) {
    var result = this.privateKeys[type];
    func(result || null);
  }
  storeSecretStorePrivateKey(_txn, type, key) {
    this.privateKeys[type] = key;
  }

  // Olm Sessions

  countEndToEndSessions(_txn, func) {
    var count = 0;
  ...
  getEndToEndSession(deviceKey, sessionId, _txn, func) {
    var deviceSessions = this.sessions[deviceKey] || {};
    func(deviceSessions[sessionId] || null);
  }
  getEndToEndSessions(deviceKey, _txn, func) {
    func(this.sessions[deviceKey] || {});
  }
  storeEndToEndSession(deviceKey, sessionId, sessionInfo, _txn) {
    var deviceSessions = this.sessions[deviceKey];
  ...
  getEndToEndInboundGroupSession(senderCurve25519Key, sessionId, _txn, func) {
  ...
  storeEndToEndInboundGroupSession(senderCurve25519Key, sessionId, sessionData, _txn) {
    var k = encodeSessionKey(senderCurve25519Key, sessionId);
