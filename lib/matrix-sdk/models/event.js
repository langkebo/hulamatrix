...
/**
 * Represents extensible event content in Matrix events.
 * The content object can have additional properties beyond the known ones.
 * We use an index signature with unknown to maintain type safety while allowing extensibility.
 */

/**
 * Represents the unsigned portion of a Matrix event.
 * Contains metadata that is not part of the event itself but is attached by the homeserver.
 */
...
export class MatrixEvent extends TypedEventEmitter {
  ...
  getContent() {
    if (this._localRedactionEvent) {
      ...
      var _ref;
      return (_ref = this._replacingEvent.getContent()["m.new_content"]) !== null && _ref !== void 0 ? _ref : {};
    } else {
  ...
  getServerAggregatedRelation(relType) {
    ...
    return relations === null || relations === void 0 ? void 0 : relations[relType];
  }
  ...
  toSnapshot() {
    ...
    for (var [p, v] of Object.entries(this)) {
      if (p !== "event") {
        ...
        // Use unknown as an intermediate type to allow dynamic property assignment
        ev[p] = v;
      }
    }
    return ev;
