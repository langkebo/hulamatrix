...
export class CallEventHandler {
  constructor(client) {
    ...
    _defineProperty(this, "onToDeviceEvent", event => {
      ...
      if (!this.nextSeqByCall.has(callId)) {
        this.nextSeqByCall.set(callId, 0);
      }
      if (content.seq === undefined) {
        this.callEventBuffer.push(event);
        return;
      }
      var nextSeq = this.nextSeqByCall.get(callId) || 0;
      var seq = content.seq;
      if (seq !== nextSeq) {
        if (!this.toDeviceEventBuffers.has(callId)) {
          this.toDeviceEventBuffers.set(callId, []);
        }
        var buffer = this.toDeviceEventBuffers.get(callId);
        var index = buffer.findIndex(e => e.getContent().seq > seq);
        if (index === -1) {
          buffer.push(event);
        } else {
        ...
        this.callEventBuffer.push(event);
        this.nextSeqByCall.set(callId, seq + 1);
  ...
  handleCallEvent(event) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var content = event.getContent();
      var confId = content.conf_id;
      var callRoomId = event.getRoomId() || confId && ((_getGroupCallById = _this2.client.groupCallEventHandler.getGroupCallById(confId)) === null || _getGroupCallById === void 0 || (_getGroupCallById = _getGroupCallById.room) === null || _getGroupCallById === void 0 ? void 0 : _getGroupCallById.roomId);
      var groupCallId = confId;
      var type = event.getType();
      var senderId = event.getSender();
      var callId = content.call_id;
      var call = callId ? _this2.calls.get(callId) : undefined;
      ...
      if (groupCallId) {
        ...
        var destSessionId = content.dest_session_id;
        if (destSessionId !== _this2.client.getSessionId()) {
      ...
      if (type === EventType.CallInvite) {
        ...
        if (lifetime && event.getLocalAge() > lifetime - RING_GRACE_PERIOD) return;
        // stale/old invite event
        if (call && call.state === CallState.Ended) return;
        if (call) {
          logger.warn("CallEventHandler handleCallEvent() already has a call but got an invite - clobbering (callId=".concat(callId, ")"));
        }
        var invitee = content.invitee;
        if (invitee && invitee !== _this2.client.getUserId()) {
        ...
        if (!call) {
          logger.log("CallEventHandler handleCallEvent() this client does not support WebRTC (callId=".concat(callId, ")"));
        ...
        if (!call) {
          ...
          var candidateCallId = content.call_id;
          if (candidateCallId && !_this2.candidateEventsByCall.has(candidateCallId)) {
            _this2.candidateEventsByCall.set(candidateCallId, []);
          }
          if (candidateCallId) {
            _this2.candidateEventsByCall.get(candidateCallId).push(event);
          }
        } else {
        ...
        if (!call) {
          ...
          if (call.state !== CallState.Ended) {
            ...
            if (call.state === CallState.Ended) _this2.calls.delete(callId);
          }
        }
        return;
      }
