...
export class MediaTrackHandler {
  ...
  getActiveSimulcastStreams() {
    // Count active transceivers that are sending/receiving media to determine simulcast layers
    // This is a simplified implementation; a more accurate count would require checking
    // the RTCRtpEncodingParameters of each transceiver's sender/receiver
    return this.pc.getTransceivers().filter(t => t.currentDirection && t.currentDirection !== "inactive").length;
  }
