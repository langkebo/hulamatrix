...
export var STORAGE_SECURITY_AUDIT = {
  logKeyOperation(operation, keyId, details) {
    logger.info("[SecurityAudit] SecretStorage key operation: ".concat(operation, ", keyId: ").concat(keyId), details);
  },
  logSecretOperation(operation, secretName, details) {
    logger.info("[SecurityAudit] SecretStorage secret operation: ".concat(operation, ", secret: ").concat(secretName), details);
  },
  warnSensitiveDataAccess(operation) {
    logger.warn("[SecurityAudit] Sensitive data access: ".concat(operation));
  }
};
