...
export class PushProcessor {
  ...
  static actionListToActionsObject(actionList) {
    ...
    for (var action of actionList) {
      if (action === PushRuleActionName.Notify) {
        ...
        var _action$value;
        (_action$value = action.value) !== null && _action$value !== void 0 ? _action$value : action.value = true;
  ...
  static rewriteDefaultRules(logger, incomingRules) {
    var _userId = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
  ...
  eventFulfillsDisplayNameCondition(_cond, ev) {
  ...
  eventFulfillsCallStartedCondition(_cond, ev) {
    ...
    return ["m.ring", "m.prompt"].includes(content["m.intent"]) && !("m.terminated" in content) && (prevContent["m.terminated"] !== content["m.terminated"] || deepCompare(prevContent, {}));
  }
