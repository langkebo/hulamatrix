...
/**
 * Delay in milliseconds before retrying a failed decryption.
 */
var DECRYPTION_RETRY_DELAY_MS = 1000; // 1 second

/**
 * @deprecated This is deprecated and not used anymore. use the ToDeviceTransport
 */
export class RoomKeyTransport extends TypedEventEmitter {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (event.isDecryptionFailure()) {
        if (!isRetry) {
          ...
          setTimeout(() => void _this.consumeCallEncryptionEvent(event, true), DECRYPTION_RETRY_DELAY_MS);
        } else {
  ...
  sendKey(keyBase64Encoded, index, _members) {
    var _this2 = this;
    return _asyncToGenerator(function* () {
