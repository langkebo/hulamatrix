    ...
    return _asyncToGenerator(function* () {
      ...
      Object.keys(wellknown).forEach(k => {
        if (k === "m.homeserver" || k === "m.identity_server") {
          ...
          var sourceConfig = wellknown[k];
          var targetConfig = clientConfig[k];
          if (sourceConfig) {
            for (var prop of Object.keys(sourceConfig)) {
              if (notProps.includes(prop)) continue;
              // Type-safe property assignment for extensible config
              targetConfig[prop] = sourceConfig[prop];
            }
          }
        } else {
