...
var KEEP_ALIVE_SUCCESS_DELAY_MS = 2000; // Delay after successful keep-alive
var KEEP_ALIVE_RETRY_BASE_DELAY_MS = 5000; // Base delay for keep-alive retry
var KEEP_ALIVE_RETRY_RANDOM_DELAY_MS = 5000; // Random delay range for keep-alive retry
    ...
    return this.client.roomInitialSync(roomId, limit).then(response => {
      ...
      if (Array.isArray(response.presence)) {
        response.presence.map(client.getEventMapper()).forEach(function (presenceEvent) {
          var userId = presenceEvent.getContent().user_id;
          var user = client.store.getUser(userId);
          if (user) {
    ...
    this.client.http.authedRequest(Method.Get, "/events", {
      ...
      var _ref6 = _asyncToGenerator(function* (res) {
        ...
        res.chunk.filter(function (e) {
          ...
          var userId = presenceEvent.getContent().user_id;
          var user = _this3.client.store.getUser(userId);
          if (user) {
            user.setPresenceEvent(presenceEvent);
          } else {
            user = User.createUser(userId, _this3.client);
            user.setPresenceEvent(presenceEvent);
  ...
  pokeKeepAlive() {
    ...
    // data
    {
      ...
      if (err.httpStatus == 400 || err.httpStatus == 404) {
        ...
        this.keepAliveTimer = setTimeout(success, KEEP_ALIVE_SUCCESS_DELAY_MS);
      } else {
        connDidFail = true;
        this.keepAliveTimer = setTimeout(this.pokeKeepAlive.bind(this, connDidFail), KEEP_ALIVE_RETRY_BASE_DELAY_MS + Math.floor(Math.random() * KEEP_ALIVE_RETRY_RANDOM_DELAY_MS));
  ...
  toDeviceMessages.map(processedMessage => {
    ...
    // For backwards compatibility, we also emit the event as a `MatrixEvent` using `ClientEvent.ToDeviceEvent`.
    {
      ...
      if (toDeviceEvent.type === "m.key.verification.start" || toDeviceEvent.type === "m.key.verification.request") {
        ...
        if (txnId && cancelledKeyVerificationTxns.includes(txnId)) {
