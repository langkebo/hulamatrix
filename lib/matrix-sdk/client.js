...
/**
 * Response from user directory list endpoint.
 */
/**
...
export class MatrixClient extends TypedEventEmitter {
  ...
  setAccountData(eventType, content) {
    ...
    return _asyncToGenerator(function* () {
      ...
      function accountDataListener(event) {
        ...
        // type matches.
        if (event.getType() === eventTypeStr) updatedResolvers.resolve();
      }
      _this8.addListener(ClientEvent.AccountData, accountDataListener);
      try {
        var result = yield retryNetworkOperation(5, () => _this8.setAccountDataRaw(eventTypeStr, content));
  ...
  /**
   * @returns Promise which resolves to the response of the state event send.
   * @returns Rejects: with an error response.
   * @throws May throw a `MatrixSafetyError` if content is deemed unsafe.
   * @see MatrixSafetyError
   */
   ...
   * @returns Promise which resolves to the response of the redaction event send.
   * @returns Rejects: with an error response.
  ...
  redactEvent(roomId, threadId, eventId, txnId, opts) {
    ...
    var reason = (_opts2 = opts) === null || _opts2 === void 0 ? void 0 : _opts2.reason;
    var content = {
      reason
    };
    if (((_opts3 = opts) === null || _opts3 === void 0 ? void 0 : _opts3.with_rel_types) !== undefined) {
   ...
   * @returns Promise which resolves to an empty object when the ban is successful.
   * @returns Rejects: with an error response.
   */
   ...
   * @returns Promise which resolves to an array of left rooms when they have been added to the data store.
   * @returns Rejects: with an error response.
   */
  syncLeftRooms() {
  ...
  /**
   * Logs out all sessions for the current user, including the current one.
   * This will revoke the user's access token and all device tokens.
   * All sessions will be terminated and the user will need to log in again.
   * @param stopClient - whether to stop the client before calling /logout/all to prevent invalid token errors.
   * @returns Promise which resolves: On success, the empty object `{}`
   */
  logoutAll() {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this56.http.authedRequest(Method.Post, "/logout/all");
    })();
  }
  ...
  /**
   * @returns Promise which resolves to an array of state events for the room.
   * @returns Rejects: with an error response.
   */
  roomState(roomId) {
   ...
   * @returns Promise which resolves to the state event content.
   * @returns Rejects: with an error response.
   */
  ...
  /**
   * @returns Promise which resolves to the room's initial sync data.
   * @returns Rejects: with an error response.
   */
   ...
   * @returns Promise which resolves to an object containing the room's visibility.
   * @returns Rejects: with an error response.
   */
  ...
  /**
   * Get the list of users from the user directory.
   *
   * @param options - Optional parameters for the request.
   * @param options.limit - The maximum number of results to return.
   * @param options.since - Pagination token to continue from a previous request.
   * @returns Promise which resolves to the user directory list response.
   */
  getUserDirectoryList() {
    ...
    return this.http.authedRequest(Method.Post, "/user_directory/list", undefined, body);
  }
   ...
   * 'avatar_url').
   * @returns Promise which resolves to the user's profile information.
   * @returns Rejects: with an error response.
   */
   ...
   *
   * @returns Promise which resolves to the response containing the sid and submission URL.
   * @returns Rejects: with an error response.
