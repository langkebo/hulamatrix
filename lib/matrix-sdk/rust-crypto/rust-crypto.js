...
export class RustCrypto extends TypedEventEmitter {
   ...
   */
  onRoomMembership(event, member, _oldMembership) {
...
class EventDecryptor {
  ...
  addEventToPendingList(event) {
    ...
    if (!roomId) return;
    var sessionId = event.getWireContent().session_id;
    if (!sessionId) return;
    var roomPendingEvents = this.eventsPendingKey.getOrCreate(roomId);
    var sessionPendingEvents = roomPendingEvents.getOrCreate(sessionId);
...
function rustEncryptionInfoToJsEncryptionInfo(_logger, encryptionInfo) {
