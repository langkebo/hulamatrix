  ...
  consumeEphemeralEvents(events) {
    events === null || events === void 0 || events.forEach(e => {
      ...
      Object.keys(e.content).forEach(eventId => {
        var eventData = e.content[eventId];
        if (!eventData) return;
        Object.entries(eventData).forEach(_ref => {
          var [key, value] = _ref;
          if (!isSupportedReceiptType(key)) return;
          if (typeof value !== "object" || value === null) return;
          for (var userId of Object.keys(value)) {
            var _eventData$key, _eventData$key2;
            var data = (_eventData$key = eventData[key]) === null || _eventData$key === void 0 ? void 0 : _eventData$key[userId];
            var receipt = {
              data: (_eventData$key2 = eventData[key]) === null || _eventData$key2 === void 0 ? void 0 : _eventData$key2[userId],
              type: key,
              eventId
            };
            ...
            // previously.
            var threadId = data === null || data === void 0 ? void 0 : data.thread_id;
            if (!threadId || typeof threadId !== "string") {
              this.setUnthreaded(userId, receipt);
            } else {
              this.setThreaded(threadId, userId, receipt);
            }
          }
        });
      });
