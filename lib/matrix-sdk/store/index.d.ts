...
/**
 * Additional events emitted by IndexedDB stores beyond the standard EventEmitter events
 */
export type StoreEvents = {
    /** Fired when the store falls back to a memory store due to errors */
    degraded: (error: Error) => void;
    /** Fired when the database closes unexpectedly */
    closed: () => void;
};
...
export interface IStore {
    ...
    on?: <K extends keyof StoreEvents | EventEmitterEvents>(event: K, handler: K extends keyof StoreEvents ? StoreEvents[K] : (...args: unknown[]) => void) => void;
