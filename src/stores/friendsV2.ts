...
export const useFriendsStoreV2 = defineStore('friendsV2', () => {
    ...
    function setupServiceListeners(): void {
        // 监听好友添加
        friendsServiceV2.on('friend.add', async (data: { friendId: string }) => {
            logger.info('[FriendsStoreV2] Friend added event received', data)
            await refreshFriends()
        })

        // 监听好友移除
        friendsServiceV2.on('friend.remove', async (data: { friendId: string }) => {
            logger.info('[FriendsStoreV2] Friend removed event received', data)
            await refreshFriends()
        })

        // 监听好友请求
        friendsServiceV2.on('request.received', async (request: PendingRequestItem) => {
            logger.info('[FriendsStoreV2] Friend request received', request)
            await refreshPending()
        })

        // 监听请求接受
        friendsServiceV2.on('request.accepted', async (data: { requestId: string; categoryId: number }) => {
            logger.info('[FriendsStoreV2] Friend request accepted', data)
            await refreshAll()
        })
    }
    ...
    async function acceptBatch(requestIds: string[], categoryId?: number): Promise<void> {
        ...
        try {
            await Promise.all(
                requestIds.map(id => friendsServiceV2.acceptFriendRequest(id, categoryId))
            )
            await refreshAll()
        } catch (e) {
    ...
    async function rejectBatch(requestIds: string[]): Promise<void> {
        ...
        try {
            await Promise.all(
                requestIds.map(id => friendsServiceV2.rejectFriendRequest(id))
            )
            await refreshPending()
        } catch (e) {
