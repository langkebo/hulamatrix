...
export const usePrivateChatStoreV2 = defineStore('privateChatV2', () => {
    ...
    function setupServiceListeners(): void {
        // 监听会话创建
        privateChatServiceV2.on('session.created', (session: PrivateChatSessionItem) => {
        ...
        // 监听会话删除
        privateChatServiceV2.on('session.deleted', (data: { sessionId: string }) => {
            logger.info('[PrivateChatStoreV2] Session deleted event received', data.sessionId)
            sessions.value = sessions.value.filter(s => s.session_id !== data.sessionId)
            messages.value.delete(data.sessionId)
        ...
        // 监听消息接收
        privateChatServiceV2.on('message.received', (message: PrivateChatMessageItem) => {
        ...
        // 监听消息发送
        privateChatServiceV2.on('message.sent', (data: { sessionId: string; messageId: string }) => {
            logger.info('[PrivateChatStoreV2] Message sent event', data.messageId)
            // 可以更新消息状态
        })
    }
