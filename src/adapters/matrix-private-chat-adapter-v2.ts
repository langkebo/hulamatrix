...
export const matrixPrivateChatAdapterV2 = {
   ...
   * 创建私聊会话
   */
  async createSession(targetId: string, sessionName?: string, ttl?: number): Promise<string> {
    try {
      ...
      // Session may have either session_id or id property
      return session.session_id || (session as { id?: string }).id || ''
    } catch (error) {
   ...
   * 获取私聊消息列表
   */
  async getMessages(sessionId: string, limit = 50, before?: string): Promise<PrivateChatMessageItem[]> {
    try {
   ...
   * 发送私聊消息
   */
  async sendMessage(sessionId: string, content: string, type: string = 'text', _ttl?: number): Promise<string> {
    try {
      if (type === 'text') {
      ...
      return await privateChatServiceV2.sendMessage({
        session_id: sessionId,
        content,
        type: type as 'file' | 'image' | 'audio' | 'video' | 'text'
      })
    } catch (error) {
   ...
   * 订阅消息更新
   */
  subscribeToMessages(sessionId: string, callback: (message: PrivateChatMessageItem) => void): () => void {
    try {
