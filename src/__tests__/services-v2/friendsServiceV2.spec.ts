...
describe('FriendsServiceV2', () => {
  ...
  beforeEach(async () => {
    ...
    const mockListFriends = vi.fn((useCache?: boolean) => {
      if (useCache && cacheData.friends) {
        return Promise.resolve(cacheData.friends)
      }
      actualApiCalls.listFriends++
      cacheData.friends = mockFriends
      return Promise.resolve(mockFriends)
    })

    const mockGetCategories = vi.fn((useCache?: boolean) => {
      if (useCache && cacheData.categories) {
        return Promise.resolve(cacheData.categories)
      }
      actualApiCalls.getCategories++
      cacheData.categories = mockCategories
  ...
  describe('List Friends', () => {
    ...
    it('should use cache when useCache is true', async () => {
      ...
      expect(actualApiCalls.listFriends).toBe(1)
    })
  ...
  describe('Get Categories', () => {
    ...
    it('should use cache when useCache is true', async () => {
      ...
      expect(actualApiCalls.getCategories).toBe(1)
    })
  ...
  describe('Send Friend Request', () => {
    ...
    it('should handle send errors', async () => {
      ...
      await friendsServiceV2.initialize()
      await expect(friendsServiceV2.sendFriendRequest('@bob:matrix.org')).rejects.toThrow('Send failed')
    })
  })
  ...
  describe('Event Handling', () => {
    it('should emit friend.add event', async () => {
      ...
      const onCallback = mockMatrixClient.friendsV2.on.mock.calls.find((call: any) => call[0] === 'friend.add')?.[1]
    ...
    it('should emit friend.remove event', async () => {
      ...
      const onCallback = mockMatrixClient.friendsV2.on.mock.calls.find((call: any) => call[0] === 'friend.remove')?.[1]
      onCallback?.({ friendId: '@alice:matrix.org' })
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid responses', async () => {
      ...
      await friendsServiceV2.initialize()
      const friends = await friendsServiceV2.listFriends()
      expect(friends).toEqual([]) // Service returns empty array for null response
    })
  })
})
