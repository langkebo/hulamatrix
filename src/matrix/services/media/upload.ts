 ...
 * ```
 */
export async function uploadContent(file: Blob | File, options?: UploadContentOptions): Promise<string> {
...
export async function downloadContentCached(
  ...
  // 存入缓存
  await hybridMediaCache.put(cacheKey, blob, options?.ttl, mxcToHttp(mxc, { authenticated: options?.authenticated }))
 ...
 * @param options - 预加载选项
 */
export async function preloadMedia(mxc: string, options?: { authenticated?: boolean; ttl?: number }): Promise<void> {
  const cacheKey = `mxc:${mxc}`
...
export function mxcToHttp(
  ...
  if (client && typeof client === 'object' && 'mxcUrlToHttp' in client && typeof client.mxcUrlToHttp === 'function') {
    return (
      client.mxcUrlToHttp as (
        mxc: string,
        width?: number,
        height?: number,
        method?: 'crop' | 'scale',
    ...
    )(mxc, options?.width, options?.height, options?.method, false, true, options?.authenticated)
  }
