...
export class MatrixReceiptsService {
  ...
  async markAsRead(roomId: string, eventId: string): Promise<void> {
    try {
      ...
      if (
        ...
        await (client.setRoomReadMarkers as (roomId: string, eventId: string, readEventId?: string) => Promise<void>)(
          roomId,
          eventId,
          eventId
        )
  ...
  getMessageReadStats(roomId: string, eventId: string): MessageReadStats {
    try {
      ...
      const room =
        client && typeof client === 'object' && 'getRoom' in client
          ? (client.getRoom as (roomId: string) => unknown)(roomId)
          : undefined
      ...
      if (
        room &&
        typeof room === 'object' &&
        'getReceiptsForEvent' in room &&
        typeof room.getReceiptsForEvent === 'function'
      ) {
        try {
              ...
              if ('getMember' in room && typeof room.getMember === 'function') {
                try {
                  ...
                  if (member && typeof member === 'object') {
                    ...
                    if (
                      'getAvatarUrl' in member &&
                      typeof member.getAvatarUrl === 'function' &&
                      client &&
                      'getBaseUrl' in client &&
                      typeof client.getBaseUrl === 'function'
                    ) {
                    ...
                    timestamp =
                      ...
                      receipt.data !== null &&
                      'ts' in receipt.data
                        ? Number((receipt.data as { ts: unknown }).ts)
                        : Date.now()
                  }
                } catch {
                  // 使用默认值
