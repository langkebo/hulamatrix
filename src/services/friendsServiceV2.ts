...
class FriendsServiceV2 implements IFriendsServiceV2 {
  ...
  private setupEventListeners(): void {
    // 监听好友添加事件
    this.friendsV2.on('friend.add', (...args: unknown[]) => {
      const data = args[0] as { friendId: string }
      logger.info('[FriendsServiceV2] Friend added event:', data.friendId)
      this.emit('friend.add', data)
    })

    // 监听好友移除事件
    this.friendsV2.on('friend.remove', (...args: unknown[]) => {
      const data = args[0] as { friendId: string }
    ...
    // 监听好友请求接收事件
    this.friendsV2.on('request.received', (...args: unknown[]) => {
      const request = args[0] as PendingFriendRequest
      logger.info('[FriendsServiceV2] Friend request received:', request.id)
      this.emit('request.received', request)
    })

    // 监听好友请求接受事件
    this.friendsV2.on('request.accepted', (...args: unknown[]) => {
      const data = args[0] as { requestId: string; categoryId: number }
   ...
   * @returns 请求 ID
   */
  async sendFriendRequest(targetId: string, message?: string, categoryId?: number): Promise<string> {
    try {
   ...
   */
  private mapToPendingItem(request: PendingFriendRequest): PendingRequestItem {
    return { ...request }
  }
  ...
  dispose(): void {
    ...
    logger.info('[FriendsServiceV2] Disposed')
  }
}
