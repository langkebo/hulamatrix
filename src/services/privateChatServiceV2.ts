...
class PrivateChatServiceV2 implements IPrivateChatServiceV2 {
    ...
    private setupEventListeners(): void {
        // 监听会话创建事件
        this.privateChatV2.on('session.created', (session: any) => {
        ...
        // 监听会话删除事件
        this.privateChatV2.on('session.deleted', (data: { sessionId: string; session?: any }) => {
            logger.info('[PrivateChatServiceV2] Session deleted:', data.sessionId)
            this.emit('session.deleted', { sessionId: data.sessionId })
        })

        // 监听消息接收事件
        this.privateChatV2.on('message.received', (message: any) => {
        ...
        // 监听消息发送事件
        this.privateChatV2.on('message.sent', (data: { sessionId: string; messageId: string }) => {
    ...
    subscribeToMessages(sessionId: string, handler: MessageHandler): () => void {
        try {
            const unsubscribe = this.privateChatV2.subscribeToMessages(
                sessionId,
                (message: any) => {
    ...
    dispose(): void {
        // 取消所有消息订阅
        for (const [sessionId, unsubscribe] of this.messageUnsubscribes.entries()) {
            unsubscribe()
        }
     ...
     * 映射到会话项目
     */
    private mapToSessionItem(session: any): PrivateChatSessionItem {
        return {
            ...session,
            ...
            participant_info: session.participant_ids?.map((p: string) => ({
                user_id: p,
                presence: 'offline'
            })) || []
        }
     ...
     * 映射到消息项目
     */
    private mapToMessageItem(message: any): PrivateChatMessageItem {
        return {
            ...message,
