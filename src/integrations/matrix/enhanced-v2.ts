...
import type { MatrixClientWithV2API } from '@/types/matrix-sdk-v2'
...
export async function setupEnhancedV2Features(): Promise<void> {
  try {
    ...
    if (!client.friendsV2 || !client.privateChatV2) {
      logger.warn('[EnhancedV2] v2 clients not available. Please ensure matrix-js-sdk version is 39.1.3 or higher')
      return
    }
...
export function getV2FeatureStatus(): {
  ...
  try {
    ...
    return {
      available: !!client.friendsV2 || !!client.privateChatV2,
      friendsAvailable: !!client.friendsV2,
      privateChatAvailable: !!client.privateChatV2
    }
  } catch {
    return { available: false, friendsAvailable: false, privateChatAvailable: false }
  }
