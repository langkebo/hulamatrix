      ...
      <div class="pt-8px flex flex-col select-none">
        <div class="flex flex-row">
            ...
            <n-scrollbar class="flex-1">
              <template v-for="session in filteredSessionList" :key="session.roomId">
                <n-flex
                  align="center"
                  :size="8"
                  class="text-12px text-var(--hula-brand-primary) dark:text-var(--hula-brand-primary) py-8px px-4px">
          ...
          <div class="flex-1 min-w-0 h-64vh px-12px pt-4px flex flex-col">
            <p class="text-(12px var(--hula-brand-primary)) pb-10px">
              {{ t('message.multi_choose.send_to_separately') }}
            </p>
            <n-scrollbar class="flex-1">
