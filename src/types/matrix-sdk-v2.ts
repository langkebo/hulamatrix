...
import type { Friend, FriendCategory } from 'matrix-js-sdk'

// ==================== Matrix Client v2 扩展接口 ====================

/**
 * FriendsClient v2 API interface
 */
export interface IFriendsClientV2 {
  listFriends(options: Record<string, unknown>, useCache?: boolean): Promise<Friend[]>
  getCategories(useCache?: boolean): Promise<FriendCategory[]>
  getPendingRequests(): Promise<PendingFriendRequest[]>
  sendFriendRequest(options: Record<string, string | number>): Promise<string>
  ...
  getFriend(userId: string): Promise<Friend | undefined>
  on(event: string, handler: (...args: unknown[]) => void): void
}

/**
 * PrivateChatClient v2 API interface
 */
export interface IPrivateChatClientV2 {
  listSessions(useCache?: boolean): Promise<PrivateChatSession[]>
  createSession(options: CreateSessionParams): Promise<PrivateChatSession>
  sendMessage(options: { session_id: string; content: string; type?: string }): Promise<string>
  sendText(sessionId: string, content: string): Promise<string>
  getMessages(options: { session_id: string; limit?: number; before?: string }): Promise<PrivateChatMessage[]>
  deleteSession(sessionId: string): Promise<void>
  subscribeToMessages(sessionId: string, handler: (message: PrivateChatMessage) => void): () => void
  invalidateCache(): void
  dispose(): void
  hasSession(sessionId: string): Promise<boolean>
  getSession(sessionId: string): PrivateChatSession | undefined
  on(event: string, handler: (...args: unknown[]) => void): void
}

/**
 * Matrix Client with v2 API extensions
 */
export interface MatrixClientWithV2API {
  friendsV2?: IFriendsClientV2
  privateChatV2?: IPrivateChatClientV2
}
...
export interface PrivateChatSession {
  session_id: string
  id?: string // SDK may return 'id' instead of 'session_id'
...
export interface PrivateChatMessage {
  message_id: string
  id?: string // SDK may return 'id' instead of 'message_id'
  session_id: string
  sender_id: string
  content: string
  type: string
 ...
 * 私聊事件类型
 */
export type PrivateChatEventType = 'session.created' | 'session.deleted' | 'message.received' | 'message.sent'
