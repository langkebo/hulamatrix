# PC 端和移动端待完善功能清单

> **最后更新**: 2026-01-06 | **文档版本**: 1.1.0
> **用途**: 汇总所有需要 PC 端和移动端实现或优化的功能

> **相关文档**:
> - [SDK 功能参考](./README.md) - Matrix JS SDK 完整功能文档
> - [后端需求汇总](./BACKEND_REQUIREMENTS.md) - Synapse Enhanced Module 开发清单
> - [SDK 集成指南](./SDK_INTEGRATION_GUIDE.md) - 本地 SDK 集成文档

---

## 📊 总体状态概览

根据 VERIFICATION 文档分析，前端整体实现状态良好，但以下功能需要 PC 端和移动端的进一步实现或优化。

| 模块 | PC 端状态 | 移动端状态 | 优先级 |
|------|----------|-----------|--------|
| 客户端基础 | ✅ 完成 | ✅ 完成 | - |
| 身份验证 | ⚠️ 缺少 UIA | ⚠️ 缺少 UIA | 高 |
| 房间管理 | ✅ 完成 | ✅ 完成 | - |
| 消息传递 | ✅ 完成 | ✅ 完成 | - |
| 事件处理 | ✅ 完成 | ✅ 完成 | - |
| 端到端加密 | ⚠️ 缺少设置界面 | ⚠️ 缺少设置界面 | 高 |
| WebRTC 通话 | ✅ 完成 | ✅ 完成 | - |
| 在线状态/输入提示 | ⚠️ 缺少 UI 组件 | ⚠️ 缺少 UI 组件 | 中 |
| 媒体文件 | ⚠️ 缺少 IndexedDB 缓存 | ⚠️ 缺少 IndexedDB 缓存 | 中 |
| 搜索功能 | ✅ 完成 | ✅ 完成 | - |
| 好友系统 | ✅ 完成 | ✅ 完成 | - |
| 私聊功能 | ✅ 完成 | ✅ 完成 | - |
| 管理员功能 | ⚠️ 缺少管理界面 | ⚠️ 缺少管理界面 | 低 |

---

## 🔴 高优先级功能

### 1. E2EE 加密设置界面 (PC + 移动)

**后端支持**: 100% ✅
**前端服务**: 100% ✅
**UI 实现**: 0% ❌

#### 需要实现的 UI 组件

##### PC 端
```typescript
// 文件位置: src/views/moreWindow/settings/E2EE.vue
// 已存在文件，需要完善功能
```

**需要添加的功能**:
1. **密钥备份状态显示**
   - 显示当前备份状态（已启用/未启用/不可用）
   - 显示备份版本号
   - 显示已备份密钥数量

2. **创建密钥备份**
   - 创建备份按钮
   - 显示恢复密钥（Recovery Key）
   - QR 码显示恢复密钥
   - 保存恢复密钥到文件

3. **恢复密钥备份**
   - 输入恢复密钥界面
   - 恢复进度显示
   - 恢复成功/失败提示

4. **设备验证**
   - 显示未验证设备列表
   - 设备验证流程
   - 设备删除功能

5. **加密模式选择**
   - 标准模式 vs 高加密模式
   - 模式切换确认

##### 移动端
```typescript
// 文件位置: src/mobile/views/settings/E2EE.vue
// 需要创建新文件
```

**需要添加的功能**:
- 同 PC 端功能，但需要适配移动端 UI
- 使用底部抽屉（BottomSheet）显示恢复密钥
- 移动端友好的设备验证界面

#### 实现参考

**后端服务 API** (已实现):
```typescript
// src/integrations/matrix/PrivateChatManager.ts

// 检查密钥备份状态
await privateChatManager.checkKeyBackupStatus()
// 返回: { enabled, version, keyCount, trustInfo }

// 创建密钥备份
await privateChatManager.createKeyBackup()
// 返回: { version, recoveryKey }

// 恢复密钥备份
await privateChatManager.restoreKeyBackup(recoveryKey)
// 返回: { imported, total }

// 检查是否需要提示用户
await privateChatManager.shouldPromptKeyBackup()
// 返回: boolean
```

#### UI 界面设计

**PC 端布局**:
```
┌─────────────────────────────────────────────────────────┐
│  端到端加密设置                              [保存] [取消] │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  密钥备份状态:  ✅ 已启用 (版本: 1, 密钥: 156)           │
│  [创建新备份]  [恢复备份]                                │
│                                                         │
│  ─────────────────────────────────────────────────────  │
│                                                         │
│  我的设备                                               │
│  ┌─────────────────────────────────────────────────┐   │
│  │ 🖥️  Windows PC                    ✅ 已验证      │   │
│  │    最后活跃: 2 分钟前                            │   │
│  │    [验证] [删除]                                 │   │
│  └─────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────┐   │
│  │ 📱  iPhone                        ⚠️  未验证      │   │
│  │    最后活跃: 1 小时前                            │   │
│  │    [验证] [删除]                                 │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│  ─────────────────────────────────────────────────────  │
│                                                         │
│  加密模式: (•) 标准模式  ( ) 高加密模式                 │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

**移动端布局**:
```
┌─────────────────────────┐
│  ← 端到端加密            │
├─────────────────────────┤
│                         │
│  密钥备份               │
│  ✅ 已启用              │
│  [创建新备份]           │
│  [恢复备份]             │
│                         │
├─────────────────────────┤
│  我的设备               │
│                         │
│  🖥️ Windows PC          │
│  ✅ 已验证              │
│  2分钟前               │
│  >                      │
│                         │
│  📱 iPhone              │
│  ⚠️ 未验证             │
│  1小时前               │
│  >                      │
│                         │
├─────────────────────────┤
│  加密模式               │
│  • 标准模式             │
│    高加密模式           │
│                         │
└─────────────────────────┘
```

---

### 2. UIA (User-Interactive Authentication) 界面

**后端支持**: 90% ⚠️
**前端服务**: 0% ❌
**UI 实现**: 0% ❌

#### 需要实现的 UI 组件

##### PC 端
```typescript
// 文件位置: src/components/auth/UIAFlow.vue
// 需要创建新文件
```

##### 移动端
```typescript
// 文件位置: src/mobile/components/auth/UIAFlow.vue
// 需要创建新文件
```

#### 功能需求

1. **多步骤认证流程**
   - 显示当前步骤
   - 显示进度指示器
   - 支持前后导航

2. **支持的认证类型**
   - 邮箱验证码
   - 手机号验证码
   - 服务条款同意
   - 密码确认
   - reCAPTCHA

3. **错误处理**
   - 显示认证失败原因
   - 支持重试
   - 超时处理

#### 实现参考

**Matrix UIA 规范**:
```typescript
// UIA 流程示例
{
  "flows": [
    {
      "stages": ["m.login.password", "m.login.terms"]
    },
    {
      "stages": ["m.login.email", "m.login.terms"]
    }
  ],
  "params": {
    "m.login.email": {
      "email": "user@example.com"
    }
  },
  "session": "session-id"
}
```

---

## 🟡 中优先级功能

### 3. 在线状态和输入指示 UI 组件

**后端支持**: 100% ✅
**前端服务**: 100% ✅
**UI 实现**: 50% ⚠️

#### 需要实现的 UI 组件

##### PC 端
```typescript
// 文件位置: src/components/common/PresenceStatus.vue
// 需要创建新文件
```

##### 移动端
```typescript
// 文件位置: src/mobile/components/common/PresenceStatus.vue
// 需要创建新文件
```

#### 功能需求

1. **在线状态指示器**
   - 显示用户在线/离线/忙碌状态
   - 绿色/黄色/灰色圆点指示
   - 悬停显示详细状态信息

2. **输入指示器**
   - 显示 "XXX 正在输入..."
   - 多个用户时显示 "XXX 和其他 2 人正在输入..."
   - 自动隐藏（超时后）

3. **已读回执**
   - 消息已读状态显示
   - 已读用户头像列表

#### 实现参考

**后端服务 API** (已实现):
```typescript
// src/services/matrixPresenceTypingService.ts

// 设置自己的在线状态
await matrixPresenceTypingService.setOnline('Available')

// 获取用户在线状态
const info = await matrixPresenceTypingService.getUserPresence(userId)
// 返回: { userId, displayName, presence, statusMsg, lastActive }

// 发送输入通知
await matrixPresenceTypingService.sendTypingNotice(roomId, true)

// 发送已读回执
await matrixPresenceTypingService.sendReadReceipt(roomId, eventId)

// 获取未读数
const { notifications, highlights } = await matrixPresenceTypingService.getRoomUnreadCount(roomId)
```

---

### 4. IndexedDB 媒体缓存

**后端支持**: 100% ✅
**前端服务**: 90% ⚠️
**UI 实现**: 不需要

#### 需要实现的服务

##### PC 端 + 移动端
```typescript
// 文件位置: src/utils/indexedDBCache.ts
// 需要创建新文件
```

#### 功能需求

1. **持久化媒体缓存**
   - 图片缓存到 IndexedDB
   - 视频缩略图缓存
   - 音频文件缓存

2. **缓存管理**
   - 设置缓存大小限制
   - LRU 淘汰策略
   - 手动清理缓存

3. **缓存统计**
   - 显示已用空间
   - 显示缓存文件数量
   - 按域名分类统计

#### 实现参考

**当前内存缓存** (已实现):
```typescript
// src/utils/download-queue.ts
export class DownloadCache {
  private cache = new Map<string, { blob: Blob; timestamp: number; expires: number }>()
  // ... 仅内存缓存
}
```

**需要实现的 IndexedDB 版本**:
```typescript
export class PersistentMediaCache {
  private db: IDBDatabase | null = null

  async init(dbName = "mediaCache", version = 1): Promise<void>
  async put(url: string, blob: Blob): Promise<void>
  async get(url: string): Promise<Blob | undefined>
  async clear(): Promise<void>
  async getStats(): Promise<CacheStats>
}

interface CacheStats {
  totalSize: number
  itemCount: number
  byDomain: Record<string, { count: number; size: number }>
}
```

---

### 5. 管理员功能界面

**后端支持**: 68% ⚠️
**前端服务**: 68% ⚠️
**UI 实现**: 0% ❌

#### 需要实现的 UI 组件

##### PC 端
```typescript
// 文件位置: src/views/admin/Dashboard.vue
// 文件位置: src/views/admin/Users.vue
// 文件位置: src/views/admin/Rooms.vue
// 文件位置: src/views/admin/Media.vue
// 需要创建这些文件
```

##### 移动端
```typescript
// 文件位置: src/mobile/views/admin/Dashboard.vue
// 需要创建这些文件
```

#### 功能需求

1. **管理员仪表板**
   - 服务器统计信息
   - 在线用户数
   - 房间数量
   - 媒体存储使用量

2. **用户管理**
   - 用户列表（分页）
   - 搜索用户
   - 修改用户权限（设为管理员/取消管理员）
   - 停用/启用用户
   - 重置用户密码
   - 删除用户

3. **房间管理**
   - 房间列表
   - 搜索房间
   - 查看房间详情
   - 删除房间

4. **设备管理**
   - 用户设备列表
   - 删除设备
   - 查看设备详情

5. **媒体管理**
   - 媒体列表
   - 隔离媒体
   - 删除媒体
   - 清理缓存

#### 实现参考

**后端服务 API** (已实现):
```typescript
// src/services/adminClient.ts

// 用户管理
await adminClient.getUser(userId)
await adminClient.listUsers({ from: 0, limit: 10 })
await adminClient.updateUserAdmin(userId, true)
await adminClient.setUserDeactivated(userId, true)
await adminClient.setUserPassword(userId, newPassword)
await adminClient.deleteUser(userId)

// 房间管理
await adminClient.getRoom(roomId)
await adminClient.listRooms({ from: 0, limit: 10 })
await adminClient.deleteRoom(roomId)

// 设备管理
await adminClient.getUserDevices(userId)
await adminClient.getDevice(userId, deviceId)
await adminClient.deleteDevice(userId, deviceId)
```

---

## 🟢 低优先级功能

### 6. 搜索结果高亮优化

**后端支持**: 100% ✅
**前端服务**: 100% ✅
**UI 实现**: 90% ✅

#### 需要优化的功能

1. **高亮样式优化**
   - 更明显的高亮颜色
   - 高亮动画效果
   - 暗色模式适配

2. **搜索结果预览**
   - 显示搜索上下文
   - 点击跳转到消息位置

---

### 7. 消息自毁倒计时样式

**后端支持**: 100% ✅
**前端服务**: 100% ✅
**UI 实现**: 90% ✅

#### 需要优化的功能

1. **倒计时动画**
   - 圆形进度条
   - 颜色渐变（绿→黄→红）
   - 倒计时数字跳动

2. **销毁动画**
   - 消失动画效果
   - 燃烧/溶解效果
   - 音效提示

---

## 📋 PC 端实现清单

### 高优先级

- [ ] **E2EE 设置界面** (`src/views/moreWindow/settings/E2EE.vue`)
  - [ ] 密钥备份状态显示
  - [ ] 创建备份界面
  - [ ] 恢复备份界面
  - [ ] 设备验证界面
  - [ ] 加密模式选择

- [ ] **UIA 认证界面** (`src/components/auth/UIAFlow.vue`)
  - [ ] 多步骤流程
  - [ ] 邮箱验证
  - [ ] 手机验证
  - [ ] 条款同意
  - [ ] 错误处理

### 中优先级

- [ ] **在线状态组件** (`src/components/common/PresenceStatus.vue`)
  - [ ] 状态指示器
  - [ ] 输入提示
  - [ ] 已读回执

- [ ] **IndexedDB 缓存** (`src/utils/indexedDBCache.ts`)
  - [ ] 持久化存储
  - [ ] 缓存管理
  - [ ] 缓存统计

- [ ] **管理员界面** (`src/views/admin/`)
  - [ ] 仪表板
  - [ ] 用户管理
  - [ ] 房间管理
  - [ ] 设备管理
  - [ ] 媒体管理

---

## 📋 移动端实现清单

### 高优先级

- [ ] **E2EE 设置界面** (`src/mobile/views/settings/E2EE.vue`)
  - [ ] 密钥备份状态显示
  - [ ] 创建备份界面（BottomSheet）
  - [ ] 恢复备份界面
  - [ ] 设备验证界面
  - [ ] 加密模式选择

- [ ] **UIA 认证界面** (`src/mobile/components/auth/UIAFlow.vue`)
  - [ ] 多步骤流程
  - [ ] 邮箱验证
  - [ ] 手机验证
  - [ ] 条款同意
  - [ ] 错误处理

### 中优先级

- [ ] **在线状态组件** (`src/mobile/components/common/PresenceStatus.vue`)
  - [ ] 状态指示器
  - [ ] 输入提示
  - [ ] 已读回执

- [ ] **IndexedDB 缓存** (`src/utils/indexedDBCache.ts`)
  - [ ] 持久化存储
  - [ ] 缓存管理
  - [ ] 缓存统计

- [ ] **管理员界面** (`src/mobile/views/admin/`)
  - [ ] 仪表板
  - [ ] 用户管理
  - [ ] 房间管理

---

## 🎯 实施建议

### Phase 1: 高优先级 (1-2 周)

1. **E2EE 设置界面**
   - PC 端: 3 天
   - 移动端: 3 天
   - 测试: 2 天

2. **UIA 认证界面**
   - PC 端: 3 天
   - 移动端: 3 天
   - 测试: 2 天

### Phase 2: 中优先级 (1-2 周)

3. **在线状态组件**
   - PC 端: 2 天
   - 移动端: 2 天

4. **IndexedDB 缓存**
   - 实现: 3 天
   - 测试: 2 天

5. **管理员界面**
   - PC 端: 5 天
   - 移动端: 3 天
   - 测试: 2 天

### Phase 3: 低优先级 (可选)

6. **搜索优化**
   - 样式优化: 1 天

7. **自毁动画**
   - 动画实现: 2 天

---

## 📚 参考文档

### UI 设计参考

- [Naive UI 组件库](https://www.naiveui.com/)
- [Material Design 移动端](https://material.io/design)
- [iOS Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines/)

### 技术参考

- [IndexedDB API](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)
- [Matrix UIA 规范](https://spec.matrix.org/v1.11/client-server-api/#user-interactive-authentication-api)
- [Matrix E2EE 实现](https://spec.matrix.org/v1.11/client-server-api/#end-to-end-encryption)

---

## 🔗 相关文档

### 项目文档
- [SDK 功能参考](./README.md) - Matrix JS SDK 完整功能文档
- [后端需求汇总](./BACKEND_REQUIREMENTS.md) - Synapse Enhanced Module 开发清单
- [SDK 集成指南](./SDK_INTEGRATION_GUIDE.md) - 本地 SDK 集成文档
- [认证分析和优化](../../AUTHENTICATION_ANALYSIS_AND_OPTIMIZATION.md) - 项目认证流程分析

### UI 设计参考

- [Naive UI 组件库](https://www.naiveui.com/)
- [Material Design 移动端](https://material.io/design)
- [iOS Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines/)

### 技术参考

- [IndexedDB API](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)
- [Matrix UIA 规范](https://spec.matrix.org/v1.11/client-server-api/#user-interactive-authentication-api)
- [Matrix E2EE 实现](https://spec.matrix.org/v1.11/client-server-api/#end-to-end-encryption)

---

**最后更新**: 2026-01-06
**文档版本**: 1.1.0
**维护者**: HuLaMatrix 开发团队

**更新内容 (v1.1.0)**:
- ✅ 更新文档日期和版本号
- ✅ 添加文档交叉引用
- ✅ 整理相关文档链接
